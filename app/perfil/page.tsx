...
export default function PerfilPage() {
  ...
  const [ciudad, setCiudad] = useState('Madrid') // NUEVO ESTADO PARA CIUDAD
    ...
    const cargarDatos = async () => {
      ...
      if (session) {
        ...
        if (data) {
          ...
          setCiudad(data.ciudad || 'Madrid') // Cargamos la ciudad si existe
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 pb-20">
      <div className="max-w-md mx-auto bg-white p-8 rounded-2xl shadow-md mt-6">
        ...
        <textarea rows={3} className="w-full p-3 mb-4 border rounded-lg outline-none text-black" value={biografia} onChange={(e) => setBiografia(e.target.value)} />

        <div className="grid grid-cols-2 gap-4 mb-8">
          <div><label className="block text-sm text-gray-700 mb-1">¿Fumas?</label><select value={fuma} onChange={(e) => setFuma(e.target.value)} className="w-full p-2 border rounded-lg bg-white text-black"><option value="No">🚭 No</option><option value="Sí">🚬 Sí</option></select></div>
          <div><label className="block text-sm text-gray-700 mb-1">¿Mascotas?</label><select value={mascotas} onChange={(e) => setMascotas(e.target.value)} className="w-full p-2 border rounded-lg bg-white text-black"><option value="No">🚫 No</option><option value="Sí">🐶 Sí</option></select></div>
          <div><label className="block text-sm text-gray-700 mb-1">Horarios</label><select value={horarios} onChange={(e) => setHorarios(e.target.value)} className="w-full p-2 border rounded-lg bg-white text-black"><option value="Madrugador">🐓 Madrugador</option><option value="Nocturno">🦉 Nocturno</option></select></div>
          <div><label className="block text-sm text-gray-700 mb-1">Limpieza</label><select value={limpieza} onChange={(e) => setLimpieza(e.target.value)} className="w-full p-2 border rounded-lg bg-white text-black"><option value="Normal">🤷‍♂️ Normal</option><option value="Estricto">✨ Estricto</option></select></div>
        </div>

        <button onClick={actualizarPerfil} disabled={loading} className="w-full bg-indigo-600 text-white p-3 rounded-lg font-bold hover:bg-indigo-700">
